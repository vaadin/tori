...
public class ActionBar extends CustomComponent {
    ...
    public ActionBar() {
        ...
        final DataSource ds = ToriApiLoader.getCurrent().getDataSource();
        if (ds.isLoggedInUser()) {
            layout.addComponent(getLink(SpecialCategory.MY_POSTS));
        }

        for (Iterator<Component> iter = layout.iterator(); iter.hasNext();) {
            layout.setComponentAlignment(iter.next(), Alignment.MIDDLE_LEFT);
        }
    }
